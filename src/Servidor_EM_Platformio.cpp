...
bool Aviso = true;
long lux_Limite = 20000;

long Luminosidad; //Para activar el endpoint y recibir un email al guardar el valor de "luminosidad"
...
BH1750 lightMeter(0x23);  //Dirección I2C - Importante, si usamos el IDE de Arduino no importará conectar el pin ADDR, en Platformio si.
...
void p_Cardinales (int valor_Grados_Pin_Veleta) 
{
//int sensorExp [] = {74, 92, 102, 134, 198, 259, 304, 427, 482, 620, 652, 725, 808, 848, 906, 963};
String dirCardinal[] = {"ESE","ENE","E","SSE","SE","SSW","S","NNE","NE","WSW","SW","NNW","N","WNW","NW","W"};
...
valor_Grados_Pin_Veleta = analogRead(direccion_Viento);
  for (int i=0; i<= 15; i++) {
    if (valor_Grados_Pin_Veleta >= sensorMin[i] && valor_Grados_Pin_Veleta <= sensorMax[i]) {
      ...
      break;  //Antes estaba comentado.
    }
  }
}

/*------------------------FIN FUNCIÓN PARA HAYAR PUNTOS CARDINALES VELETA------------------------*/
...
void leer_Sensores ()
{
  ...
  Serial.print (angulo);
  Serial.println ("º"); 
  Serial.print ("El punto cardinal del viento es: ");
...
void setup()
{
  ...
  Wire.begin (D2, D1); //pin SDA y pin SCL
  ads.begin();  //Iniciamos el sensor ML8511 que está conectado al ADS1115.

  lightMeter.begin(BH1750::CONTINUOUS_HIGH_RES_MODE); //Modo alta resolución "MODO CONTINUO"

  if (lightMeter.begin(BH1750::CONTINUOUS_HIGH_RES_MODE)) { //begin devuelve un boolean que puede ser usado para detectar problemas de instalación.
  ...
  thing.add_wifi(WiFi_ssid, WiFi_password); // Inicialización de la red WiFi para comunicarse con la API
  Serial.println(" ");
...
void loop()
{
  ...
  if (currentMillis - previousMillis_Lecturas > intervalo_entre_Lecturas) //Si la resta es > que intervalo_entre_Lecturas, vuelve a leer los sensores
  {
    ...
    p_Cardinales (valor_Grados_Pin_Veleta);
    getRain();
